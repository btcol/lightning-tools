...
1. **🌐 Red & Cockpit HUD**: Orquesta el escaneo del public graph. Despliega un modelo orbital astronómico 3D de tu nodo e integra la generación de un Cockpit HTML interactivo (Heads-Up Display) para monitorear en tiempo real métricas clave de rentabilidad, canales zombies, liquidez y uptime basados en tu historial.
2. **👛 Wallet On-chain**: Control centralizado de tu liquidez en capa base. Permite monitorear balances confirmados/no-confirmados, generar nuevas direcciones, detectar alta fragmentación y consolidar UTXOs con comisiones personalizadas. También gestiona la exportación manual y visualiza el estado de los Backups Estáticos de Canales (SCB).
3. **👥 Gestión de Peers**: Vista en tabla de tus conexiones P2P públicas en la red.
