...
# BASE_DIR apunta a lightning-web/, por lo que data/, exports/, backups/ son propios.
...
@requires_auth
# Endpoint: Devuelve el estado actual de ejecucion del piloto automatico de rebalanceo.
def api_autopilot_status():
    return jsonify({"active": _autopilot_active})

@app.route("/api/rebalance/autopilot/stop", methods=["POST"])
@requires_auth
# Endpoint: Envia la senal de parada segura al thread del piloto automatico.
...
@requires_auth
# Endpoint: Inicia el bucle iterativo experimental del piloto automatico via SSE.
def api_rebalance_autopilot():
    ...
    El cliente puede detenerlo enviando POST /api/rebalance/autopilot/stop.
    """
